...
- [x] 3.1 `i` on a requirement row and on a change heading: write prompt
      file under the state directory, suspend, run, resume with the
      selection kept.

## 4. Batch

- [x] 4.1 `A` on a pairing, `Shift-A` on a change: worker thread,
      spinner and counter in the status line, sequential agent calls.
- [x] 4.2 Reply parser to hints; `Unparsed` on malformed replies;
      stderr to the status line on non-zero exit.
- [x] 4.3 Hint severity below note; `✦` row marker; detail pane section;
      exit status ignores hints.
- [x] 4.4 `--assist` on plain runs; `--hints` to include hints in
      `--findings-only`.

## 5. State

- [x] 5.1 Hint cache keyed by the normalized-after-text hash; re-run
      replaces.
- [x] 5.2 Dismissal by kind and message with `x`; hidden in view and
